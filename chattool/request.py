...
## Requests for finetune API
def upload_file( api_key:str
               , base_url:str
               , purpose:str
               , filename:str):
    """Upload a file to the OpenAI server
    ...
    Args:
        ...
        filename (str): Name of the JSON Lines file to be uploaded.

    Returns:
        str: file id
    """
    headers = {"Authorization": "Bearer " + api_key}
    files_url = normalize_url(os.path.join(base_url, "v1/files"))
    data = {"purpose": purpose}
    with open(filename, "rb") as f:
        response = requests.post(files_url, headers=headers, data=data, files={"file": f})
    if response.status_code == 200:
        return response.json()
    else:
        raise Exception(response.text)

def get_file_list(api_key:str, base_url:str):
    """Get file list
    Request url: https://api.openai.com/v1/files
